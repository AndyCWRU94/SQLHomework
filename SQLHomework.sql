...
SELECT * FROM sakila.actor WHERE actor.last_name LIKE '%GEN%';

# 2c. Find all actors whose last names contain the letters LI. This time, 
# order the rows by last name and first name, in that order:
SELECT * FROM sakila.actor WHERE actor.last_name LIKE '%LI%' ORDER BY actor.last_name, actor.first_name;
...
# be performing queries on a description, so create a column in the table actor named 
...
SELECT DISTINCT actor.last_name, COUNT(last_name) AS 'name_count'
FROM sakila.actor
GROUP BY last_name;
...
FROM
    sakila.staff
        INNER JOIN
    sakila.address ON staff.address_id = address.address_id 
		INNER JOIN
...
FROM
    sakila.customer
        INNER JOIN
    sakila.customer_list ON customer.customer_id = customer_list.ID
WHERE
...
FROM
    ...
    sakila.staff ON store.store_id = staff.store_id
        INNER JOIN
    sakila.payment ON payment.staff_id = staff.staff_id
...
FROM
    sakila.category c
        INNER JOIN
    sakila.film_category fc ON fc.category_id = c.category_id
        INNER JOIN
    sakila.inventory i ON i.film_id = fc.film_id
        INNER JOIN
    sakila.rental r ON r.inventory_id = i.inventory_id
        RIGHT JOIN
    sakila.payment p ON p.rental_id = r.rental_id
GROUP BY name
